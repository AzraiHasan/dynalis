// Simplified demo data composable with localStorage
import { ref, computed, readonly } from 'vue'
import { useDemoSession } from './useDemoSession'
import type { DemoSite } from '~/types/demo'

// Sample Malaysian property data for demo
const DEMO_SITES_DATA: Omit<DemoSite, 'id' | 'created_at' | 'updated_at'>[] = [
  {
    site_id: 'KL001',
    exp_date: '2024-12-31',
...
export const useDemoData = () => {
  ...
  const initializeDemoData = () => {
    ...
    const demoSites: DemoSite[] = DEMO_SITES_DATA.map((site, index) => ({
      ...site,
  ...
  const simulateUpload = async (fileData: Record<string, string | number>[] = []): Promise<void> => {
    ...
    for (let i = 0; i < dataToProcess.length; i++) {
      ...
      const newSite: DemoSite = {
        ...
        site_id: String(dataToProcess[i].site_id || `UP${String(i + 1).padStart(3, '0')}`),
        exp_date: String(dataToProcess[i].exp_date || '2025-12-31'),
        total_rental: Number(dataToProcess[i].total_rental || Math.random() * 3000 + 1000),
        total_payment_to_pay: Number(dataToProcess[i].total_payment_to_pay || Math.random() * 3000 + 1000),
        deposit: Number(dataToProcess[i].deposit || Math.random() * 500 + 100),
        lot_no: String(dataToProcess[i].lot_no || `LOT-${i + 1}`),
        location: String(dataToProcess[i].location || 'Demo Location'),
        tenant_name: String(dataToProcess[i].tenant_name || `Tenant ${i + 1}`),
        contact_no: String(dataToProcess[i].contact_no || '+60123456789'),
  ...
  // Add individual site
  const addSite = (siteData: Omit<DemoSite, 'id' | 'created_at' | 'updated_at'>) => {
    if (!demoSession.isDemoMode()) return
  ...
  // Update site
  const updateSite = (siteId: string, updates: Partial<DemoSite>) => {
    if (!demoSession.isDemoMode()) return
  ...
  const clearDemoData = () => {
    ...
    if (import.meta.client) {
      localStorage.removeItem('demo_sites_data')
    }
  ...
  // Storage helpers
  const storeDemoData = (data: DemoSite[]) => {
    if (!import.meta.client) return
    try {
      localStorage.setItem('demo_sites_data', JSON.stringify(data))
    } catch (error) {
      console.warn('[Demo Data] Failed to store data:', error)
    }
  }
  
  const getStoredDemoData = (): DemoSite[] => {
    if (!import.meta.client) return []
    try {
      const stored = localStorage.getItem('demo_sites_data')
